...
library(needs)
needs(xfun, tidyverse, remotes, devtools, mice, pastecs,svglite, HLMdiag, gtsummary, cardx, flextable, lme4, nlme, pwr,huxtable, broom.mixed, patchwork, sjPlot, ggcorrplot, lmerTest)
...
df_less_na<-df_filtered %>% filter(Missings_amount<20)#removing participants with more than 50 missings
...
# #df<-complete(imp)
# 
#load(url("https://github.com/Enno-W/Self-Conscious-Emotions-in-Sport-and-Exercise/blob/main/df_imputed.Rdata?raw=true"))# outcomment this to NOT use imputation
...
icc_goal<--performance::icc(null_model_km)
...
# km model with lme4
model_SessionKM_lme4<- lme4::lmer(SessionKM~ Pride * Dynamics + Pride * PositiveAffect + Pride * Locus + Pride * Globality + Pride * 
Affiliation + Pride * Achievement + Pride * Power + (1 | ID), data = long_df)

model_with_pvals <- as_lmerModLmerTest(model_SessionKM_lme4)
summary(model_with_pvals)

model_SessionKM_lme4<- lme4::lmer(SessionH~ Pride * Dynamics + Pride * PositiveAffect + Pride * Locus + Pride * Globality + Pride * 
Affiliation + Pride * Achievement + Pride * Power + (1 | ID), data = long_df)
...
ggplot(data = model_SessionKM_residuals , aes( x= .ls.resid, y= as.factor(ID))) + theme_gray() + geom_boxplot() # per group, the outliers are pretty random and the width varies substantially between groups
...
Homoscedasticity_plot <-plot_model(model_SessionKM, type = "diag") 
