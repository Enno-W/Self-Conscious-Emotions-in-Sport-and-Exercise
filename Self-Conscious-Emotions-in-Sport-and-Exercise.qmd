...
library(needs)
needs(xfun, tidyverse, remotes, devtools, mice, pastecs,svglite, HLMdiag, gtsummary, cardx, flextable, lme4, nlme, pwr,huxtable, broom.mixed, patchwork, sjPlot, ggcorrplot, lmerTest, MuMIn, mlmhelpr)
...
sum(is.na(df[,c("WeeklyKM_base", "SessionKM_1", "SessionKM_2", "SessionKM_3", "SessionKM_4", "SessionKM_5", "SessionKM_6")] ))
df <- df %>%
  group_by(SportCode) %>%
  mutate(across(c(WeeklyKM_base, SessionKM_1, SessionKM_2, SessionKM_3, SessionKM_4, SessionKM_5, SessionKM_6),
                ~ scale(.x)))
sum(is.na(df[,c("WeeklyKM_base", "SessionKM_1", "SessionKM_2", "SessionKM_3", "SessionKM_4", "SessionKM_5", "SessionKM_6")] ))

sum(is.na(df[,c("WeeklyKM_base", "SessionH_1", "SessionH_2", "SessionH_3", "SessionH_4", "SessionH_5", "SessionH_6")] ))
df <- df %>%
  group_by(SportCode) %>%
  mutate(across(c(WeeklyH_base, SessionH_1, SessionH_2, SessionH_3, SessionH_4, SessionH_5, SessionH_6),
                ~ scale(.x)))
sum(is.na(df[,c("WeeklyKM_base", "SessionH_1", "SessionH_2", "SessionH_3", "SessionH_4", "SessionH_5", "SessionH_6")] ))
...
weekly_measures<-df %>% ungroup() %>% select( matches("_[1-6]$")) %>% names()# This is a regex, a regular expression to find a certain pattern. The Dollar sign is for "Ends with". Learn more here: https://github.com/ziishaned/learn-regex/blob/master/translations/README-de.md
...
long_df <- long_df %>%
  group_by(ID) %>%
  mutate(across(c(Pride, PA, Pride,PA), 
                ~ . - mean(., na.rm = TRUE), 
...
SessionKM_residuals_no_logtransform <- hlm_resid(model_SessionKM_4, level=1, include.ls = T) 
########Normality Plots #####
...
#ndist_plot_km+ndist_plot_km_no_logtransform
# QQ line
...
qqplot_km_no_logtransform <-ggplot(SessionKM_residuals_no_logtransform, aes(sample = .resid)) +
...
# qqplot_km+qqplot_km_no_logtransform
...
ntest_ks_km_no_logtransform <-ks.test(SessionKM_residuals_no_logtransform  $.resid, "pnorm", mean(SessionKM_residuals_no_logtransform  $.resid), sd(SessionKM_residuals_no_logtransform  $.resid), exact = T)
...
resid_plot_km_no_logtransform<-ggplot(data = SessionKM_residuals_no_logtransform, aes(x = .fitted, y = .resid)) +
