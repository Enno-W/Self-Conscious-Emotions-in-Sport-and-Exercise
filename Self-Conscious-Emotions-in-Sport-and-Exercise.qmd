...
long_df <- long_df %>%
  ...
  ungroup()
long_df$SessionKM_rank<-rank(long_df$SessionKM)
long_df$SessionH_rank<-rank(long_df$SessionH)
long_df$SessionKM_log <- log(long_df$SessionKM)
long_df$SessionKM_log[is.infinite(long_df$SessionKM_log)] <- NA
long_df$SessionH_log <- log(long_df$SessionH)
long_df$SessionH_log[is.infinite(long_df$SessionH_log)] <- NA
long_df$SessionRPE_log <- log(long_df$SessionRPE)
long_df$SessionRPE_rank <- rank(long_df$SessionRPE)

long_df$ID<-long_df$ID %>% as.factor()
```

```{r HLM Table and graphs for KM}
#| include: false


##### Null model and ICC ####

null_model_km<- lme(SessionKM_log ~ 1, 
                      data=long_df, 
                      random= ~1|ID, 
                      method="ML", 
...
# 
# model_SessionKM_log<-lmer(SessionKM_log~ Pride * Dynamics + Pride * PA + Pride * Locus + Pride * Globality + Pride * Affiliation + 
...
##### Null model and ICC ####
null_model_h<- lme(SessionH_log ~ 1, 
                      data=long_df, 
                      random= ~1|ID, 
                      method="ML", 
                      na.action = na.omit) # See the documentation for lme: ?lme --> other options: na.exclude, na.pass...#
...
```{r HLM Table and graphs for RPE}
#| include: false


##### Null model and ICC ####

null_model_rpe<- lme(SessionRPE_log ~ 1, 
                      data=long_df, 
                      random= ~1|ID, 
                      method="ML", 
                      na.action = na.omit) # See the documentation for lme: ?lme --> other options: na.exclude, na.pass...#
...
residuals_model <- residuals(model_SessionKM_4)
hist(residuals_model, breaks = 30, main = "Histogram of Residuals", xlab = "Residuals")
qqnorm(residuals_model)
qqline(residuals_model, col = "red")
shapiro.test(residuals_model)
#ks.test(residuals_model, "pnorm", mean(residuals_model), sd(residuals_model))

##### Checking Assumptions http://www.regorz-statistik.de/inhalte/r_HLM_2.html ###############################################
model_SessionKM_residuals  <- hlm_resid(model_SessionKM_4, level=1, include.ls = T) # Funktion aus HLMdiag-Package
