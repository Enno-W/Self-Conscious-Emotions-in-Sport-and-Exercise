...
### For weekly variables ##

for (i in 1:6) {
  speed_var <- paste0("Speed_", i)
  class_var <- paste0("Speed_", i, "_class")
  z_var     <- paste0("Speed_", i, "_z")
  
  df <- df %>%
    group_by(.data[[class_var]]) %>%
    mutate(!!z_var := as.numeric(scale(.data[[speed_var]]))) %>%
    ungroup()
}

df <- df %>%
  group_by(Speed_1_class) %>%
  mutate(Speed_1_ztest = as.numeric(scale(Speed_1))) %>%
  ungroup()
df$Speed_1_z == df$Speed_1_ztest
...
null_model_km<- lme(SessionKM_lead1 ~ 1, 
                      data=long_df %>% filter(SportCode!=2), 
                      random= ~1|ID, 
                      method="ML", 
...
# Extract Residuals
SessionKM_residuals <- hlm_resid(model_SessionKM_1, level=1, include.ls = T) 
########Normality Plots #####
