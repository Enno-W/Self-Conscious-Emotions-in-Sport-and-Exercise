...
ndist_plot_km <- ggplot(data = SessionKM_residuals  , aes(.ls.resid)) +
...
ndist_plot_km <- ggplot(data = SessionKM_residuals  , aes(.resid)) +
  geom_histogram(aes(y = after_stat(density)), bins=30) +
  stat_function(fun = dnorm,
                args = list(mean = mean(SessionKM_residuals  $.resid),
                            sd = sd(SessionKM_residuals  $.resid)), linewidth=2) 
...
ntest_shapiro_km_p <-if (ntest_shapiro_km$p.value < 0.001) {
  "< .001"
} else {
