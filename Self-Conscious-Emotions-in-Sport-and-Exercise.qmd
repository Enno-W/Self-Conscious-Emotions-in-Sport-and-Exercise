...
original_data_missings_amount<-sum(is.na(df))

#### counting excluded participants ####
raw_data_n <-nrow(df)

#### Removing Participants based on the filter varialbe "Programme"#####
df_filtered<-df %>% filter(Programme == 1)
filtered_n <-nrow(df_filtered)

# ID Variable
df_filtered$ID<-1:nrow(df_filtered)

#### Grouping the different kinds of sports and goals ####
# df_filtered$Sport2 <- NA
# df_filtered$Sport2[19] <- "Laufen" # Here, I add a second sport for participant 19
# patterns <- c(  "Kraftsport" = "kraft",   "Laufen" = "lauf")
# df <- replace_patterns(df_filtered, "Sport", patterns)# See the script "Functions.R" to examine the function

#### Removing participants that don't have "Triathlon" or "Laufen" as sport ####
# df<-df %>% filter(Sport == "Triathlon"| Sport== "Laufen")

#### NAs and Outliers ####
df$Missings_amount <- rowSums(is.na(df))
df<-df %>% filter(Missings_amount<50)#removing participants with more than 50 missings
na_removed_n <-nrow(df)
...
# z-Standardizing base_km

# z-Standardizing Implicit Motives data

df[,c("Affiliation_zscore", "Achievement_zscore", "Power_zscore")]<-scale(df[,c("Affiliation", "Achievement", "Power")])
...
model_SessionH <- lme(
  fixed = SessionH ~ Pride * Dynamics + Pride * PositiveAffect + Pride * Locus + Pride * Globality + 
          Pride * Affiliation + Pride * Achievement + Pride * Power,
  random = ~ 1 | ID,
   correlation = corCAR1(form = ~ Time | ID),
  data = long_df, 
...
null_model_km <- lmer(SessionKM ~ 1 + (1 | ID), 
                      ...
                      na.action = na.omit)
summary(null_model_km)
model_SessionKM_lme4<- lmer(SessionKM ~ Pride * Dynamics + Pride * PositiveAffect + Pride * Locus + Pride * Globality + Pride *
       Affiliation+ Pride * Achievement + Pride * Power + (1 | ID), data = long_df)
summary(model_SessionKM_lme4)
r.squaredGLMM(model_SessionKM)

# Grand mean centering
long_df[, c("Pride_centered", "Dynamics_centered", "PositiveAffect_centered", "Locus_centered", "Globality_centered", "Affiliation_centered", "Achievement_centered", "Power_centered")] <- scale(long_df[, c("Pride", "Dynamics", "PositiveAffect", "Locus", "Globality", "Affiliation", "Achievement", "Power")], center = TRUE, scale = FALSE)

model_SessionKM_lme4_grand_centered<- lmer(SessionKM ~ Pride_centered * Dynamics_centered + Pride_centered * PositiveAffect_centered + Pride_centered * Locus_centered + Pride_centered * Globality_centered + Pride_centered *
...
long_df <- long_df %>%
  group_by(ID) %>%
  mutate(across(c(Pride, Dynamics, PositiveAffect, Locus, Globality, 
                  Affiliation, Achievement, Power), 
                ~ . - mean(.), 
                .names = "{.col}_centered")) %>%
  ungroup()

model_SessionKM_lme4_group_centered<- lmer(SessionKM ~ Pride_centered * Dynamics_centered + Pride_centered * PositiveAffect_centered + Pride_centered * Locus_centered + Pride_centered * Globality_centered + Pride_centered *
       Affiliation + Pride_centered * Achievement + Pride_centered * Power + (1 | ID), data = long_df)
...
model_SessionH_lme4 <- lmer(SessionH ~ Pride * Dynamics + Pride * PositiveAffect + Pride * Locus + Pride * Globality + Pride * Affiliation +
...
The original dataset contained data from `r raw_data_n` participants, of which `r raw_data_n - filtered_n` were excluded because they reported that they are not in a training programme. After that, participants with more than 30 missing were excluded. This ensured that each participants completed the questionnaire for at least two sessions, and led to a final sample size of `r na_removed_n`. 
