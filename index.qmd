...
Exploratory analyses revealed significant correlations between pride and positive affect (*Ρ* = `r cor.test(df_study1$Pride_base, df_study1$PA_base)$estimate %>% round (2)`, *p*   
`r cor.test(df_study1$Pride_base, df_study1$PA_base)$p.value %>% round (3) %>% format_p()`), weekly training duration and training distance (*Ρ* = 
`r cor.test(df_study1$WeeklyH_base, df_study1$WeeklyKM_base)$estimate %>% round (2)`, *p*  
`r cor.test(df_study1$WeeklyH_base, df_study1$WeeklyKM_base)$p.value %>% round (3) %>% format_p()`), the implicit achievement and affiliation motives (*Ρ* = 
`r cor.test(df_study1$Achievement, df_study1$Affiliation)$estimate %>% round (2)`, *p*  
`r cor.test(df_study1$Achievement, df_study1$Affiliation)$p.value %>% round (3) %>% format_p()`), hope for success and hope for belonging (*Ρ* = 
`r cor.test(df_study1$Success_hope, df_study1$Belonging_hope)$estimate %>% round (2)`, *p*  
`r cor.test(df_study1$Success_hope, df_study1$Belonging_hope)$p.value %>% round (3) %>% format_p()`), hope for success and hope for control (*Ρ* = 
`r cor.test(df_study1$Success_hope, df_study1$Control_hope)$estimate %>% round (2)`, *p*  
`r cor.test(df_study1$Success_hope, df_study1$Belonging_hope)$p.value %>% round (3) %>% format_p()`), fear of failure and fear of social rejection (*Ρ* = 
`r cor.test(df_study1$Failure_fear, df_study1$Rejection_fear)$estimate %>% round (2)`, *p*  
`r cor.test(df_study1$Failure_fear, df_study1$Rejection_fear)$p.value %>% round (3) %>% format_p()`), fear of failure and fear of loss of control (*Ρ* = 
`r cor.test(df_study1$Failure_fear, df_study1$LossControl_fear)$estimate %>% round (2)`, *p*  
`r cor.test(df_study1$Failure_fear, df_study1$LossControl_fear)$p.value %>% round (3) %>% format_p()`), fear of social rejection and negative affect (*Ρ* = 
`r cor.test(df_study1$Rejection_fear, df_study1$NA_base)$estimate %>% round (2)`, *p*  
`r cor.test(df_study1$Rejection_fear, df_study1$NA_base)$p.value %>% round (3) %>% format_p()`), and uncontrollable and internal attributions (*Ρ* = 
`r cor.test(df_study1$Controlability_self, df_study1$Locus)$estimate %>% round (2)`, *p* 
`r cor.test(df_study1$Controlability_self, df_study1$Locus)$p.value %>% round (3) %>% format_p()`).
...
nlme::Variogram(model_SessionRPE_3)
