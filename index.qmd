...
We assumed that the measurements at each point were intercorrelated, leading to autoregressive noise. Using the `variogram ()`- function of {nlme}, we examined variograms to assess the correlation structure between measurement points. The results suggested that autoregressive noise was present: The semivariance changed substantially with distance between observations. Thus, we included an autocorrelation structure of order 1 (AR1) in the models.
