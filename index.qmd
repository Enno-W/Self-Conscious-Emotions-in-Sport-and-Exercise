...
get_descriptive_table <- function(df, language = "German") {
  ...
  df_stat <- df %>% 
    ...
    t() %>% 
    as.data.frame() %>% 
    select(-var, -coef.var, -SE.mean, -kurt.2SE, -normtest.W, -skew.2SE, -normtest.p)
...
Exploratory analyses revealed significant correlations between pride and positive affect (*Ρ* = `r cor.test(df_study1$Pride_base, df_study1$PA_base)$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Pride_base, df_study1$PA_base)$p.value %>% round (3) %>% format_p()`), weekly training duration and training distance (*Ρ* = `r cor.test(df_study1$WeeklyH_base, df_study1$WeeklyKM_base)$estimate %>% round (2)`, *p*  `r cor.test(df_study1$WeeklyH_base, df_study1$WeeklyKM_base)$p.value %>% round (3) %>% format_p()`), the implicit achievement and affiliation motives (*Ρ* = `r cor.test(df_study1$Achievement, df_study1$Affiliation)$estimate %>% round (2)`, *p*  `r cor.test(df_study1$Achievement, df_study1$Affiliation)$p.value %>% round (3) %>% format_p()`), hope for success and hope for belonging (*Ρ* = 
`r cor.test(df_study1$Success_hope, df_study1$Belonging_hope)$estimate %>% round (2)`, *p*  `r cor.test(df_study1$Success_hope, df_study1$Belonging_hope)$p.value %>% round (3) %>% format_p()`), hope for success and hope for control (*Ρ* = `r cor.test(df_study1$Success_hope, df_study1$Control_hope)$estimate %>% round (2)`, *p*  `r cor.test(df_study1$Success_hope, df_study1$Belonging_hope)$p.value %>% round (3) %>% format_p()`), fear of failure and fear of social rejection (*Ρ* = `r cor.test(df_study1$Failure_fear, df_study1$Rejection_fear)$estimate %>% round (2)`, *p*  `r cor.test(df_study1$Failure_fear, df_study1$Rejection_fear)$p.value %>% round (3) %>% format_p()`), fear of failure and fear of loss of control (*Ρ* = `r cor.test(df_study1$Failure_fear, df_study1$LossControl_fear)$estimate %>% round (2)`, *p*  `r cor.test(df_study1$Failure_fear, df_study1$LossControl_fear)$p.value %>% round (3) %>% format_p()`), fear of social rejection and negative affect (*Ρ* = `r cor.test(df_study1$Rejection_fear, df_study1$NA_base)$estimate %>% round (2)`, *p*  `r cor.test(df_study1$Rejection_fear, df_study1$NA_base)$p.value %>% round (3) %>% format_p()`), and uncontrollable and internal attributions (*Ρ* = `r cor.test(df_study1$Controlability_self, df_study1$Locus)$estimate %>% round (2)`, *p* `r cor.test(df_study1$Controlability_self, df_study1$Locus)$p.value %>% round (3) %>% format_p()`).
...
We excluded participants manually if they reported data on less than two training sessions, if they did not participate in a structured training programme, or if they were neither runners nor triathletes. 
 `r missings_percentage_study2` of data were missings. Since most of the incomplete date was due to dropouts, we decided against multiple imputation, following the reccomendations of \[Lit\]. 
 
 We calculated the speed from the time and distance values for each session, and found that the speed in a session ranged from `r min(c(descriptives_list_study2$Speed_1_min, descriptives_list_study2$Speed_2_min, descriptives_list_study2$Speed_3_min, descriptives_list_study2$Speed_4_min, descriptives_list_study2$Speed_5_min, descriptives_list_study2$Speed_6_min), na.rm = TRUE) %>% round(0)` to `r max(c(descriptives_list_study2$Speed_1_max, descriptives_list_study2$Speed_2_max, descriptives_list_study2$Speed_3_max, descriptives_list_study2$Speed_4_max, descriptives_list_study2$Speed_5_max, descriptives_list_study2$Speed_6_max), na.rm = TRUE) %>% round(0)` kilometers per hour. Thus, we separated the data into three categories: If the speed was less than five kilometers per hour, the session was regarded as "swimming", less than 20 kilometers per hour as "running", and more than 20 kilometers per hour as "cycling". These cutoffs reflect typical training speeds for each sport. We then z-standardized the session distance within these groups to make the distances comparable between different sports.  
 
 
Since the questionnaires were sent out in weekly intervals, we expected autocorrelation between measurement points. Through inspecting variograms, we assessed the correlation structure between measurement points. The results suggested that autoregressive noise was present: The semivariance changed substantially with distance between observations. Thus, we included an autocorrelation structure of order 1 (AR1) in the models.
...
In these models, we used interaction terms of pride with positive affect, causal attribution and implicit motives to predict running distance. The running distance was z-standardized to allow a comparison between the kilometer values reported by swimmers, runners, and triathletes. We added predictors to the null model in three steps: First, the interaction term of pride and positive affect, second, the interaction terms of pride and attribution styles, and third, interaction terms of pride and implicit motives. The result is shown in @tbl-hlmtable_km.

The intraclass correlation coefficient was *ICC* = `r icc_km[1]$ICC_adjusted %>% round (2)`. 
Model 3 included meaningful predictors and the lowest log-likelihood, it also fit significantly better than the same model without a correlation matrix *p*  `r corAR_test_km$"p-value"[2] %>% format_p()`. Thus, Model 3 was selected for further analysis of statistical assumptions and the discussion. 

The residuals were normally distributed (*p*  `r ntest_shapiro_km  $p.value %>% format_p()` ). @fig-sessionkm_assumptions shows a QQ-plot and scatterplot of the residuals. Here, the residuals are concentrated in the left, indicating a slight deviation from the assumption of homoscedasticity. 
...
On the individual level (level 2), the residuals were not normally distributed (*p*  `r level2.resids.shapirotest$p.value %>% round(3)`). No deviation from the assumption of homoscedasticity was present (*p*  `r bp_test_km$p.value[1] %>% round(3) `). 
...
The interactions between pride and positive affect (*b* = `r b_prideXpa_km %>% round(3)`, *p* `r b_prideXpa_km %>% format_p()`), and achievement motive (*b* = `r b_prideXachievement_km %>% round(3)`, *p*  `r b_prideXachievement_km %>% format_p()`) were positive, meaning that the effect of pride on the training distance in the *next* session is stronger when there is a high achievement motive or a high positive affect (in the previous session).
The interaction terms between pride and globality (*b* = `r b_prideXpa_km %>% round(3)`, *p* `r b_prideXpa_km %>% format_p()`), the affiliation motive (*b* = `r b_prideXaffiliation_km %>% round(3)`, *p* = `r b_prideXaffiliation_km %>% format_p()`) and the power motive (*b* = `r b_prideXpower_km %>% round(3)`, *p*  `r b_prideXpower_km %>% format_p()`) were negative: When globality, affiliation or power are high, pride reduces the running distance in the next session. 
...
No model showed meaningful predictors of session duration. A correlation matrix only slightly and non-significantly improved model fit (*p*  `r corAR_test_h$"p-value"[2] %>% format_p()`). The model with the correlation matrix (Model 3) was retained for further examination. 

The residuals were normally distributed in a Kolmogorov-Smirnov-Test (*p*  `r ntest_ks_h $p.value %>% format_p()` ), but not in a Shapiro-Wilk-test (*p*  `r ntest_shapiro_h$p.value %>% format_p()`). A slight deviation from the normal distribution of residuals and the assumption of homoscedasticity is apparent in @fig-sessionh_assumptions. 
...
On the individual level (level 2), the residuals were not normally distributed (*p*  `r level2.resids.shapirotest_h$p.value %>% format_p()`). No deviation from the assumption of homoscedasticity was present (*p*  `r bp_test_h$p.value[1] %>% format_p() `). 
...
No predictors predicted session time significantly. 
...
The regression coefficients are shown in @tbl-hlmtable_rpe.  The third model with all predictors shows the lowest AIC and log likelihood. A correlation matrix did not result in a better fit ( *p*  `r intercorr_test_rpe$"p-value"[2] %>% round(2)`), so we used the model without the correlation structure. It shows the lowest AIC and log likelihood and was tested for statistical assumptions and model quality.
...
The residuals were normally distributed in a Kolmogorov-Smirnov test (*p*  `r ntest_ks_rpe $p.value %>% format_p()` ),  but not in a Shapiro-Wilk-test (*p*  `r ntest_shapiro_rpe$p.value %>% format_p()`). @fig-sessionrpe_assumptions shows the deviation from the normal distribution in a QQ-Plot along with the distribution of residuals. The linear patterns that emerge in the residuals are a result of the discrete values in the session RPE data.
