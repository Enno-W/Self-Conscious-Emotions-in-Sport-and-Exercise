...
model_SessionKM_1 <- lme(
  ...
  random = ~ 1 | ID,
  correlation = nlme::corCAR1(form = ~ Time | ID),
  data = long_df,
  na.action = na.omit,
  method = "REML"
)
...
model_SessionKM_3_no_corr_matrix <- lme(
  fixed = SessionKM_lead1 ~ Pride_cm_centered * PA_cm_centered +
    Pride_cm_centered * Dynamics_centered +
    Pride_cm_centered * Locus_centered +
    Pride_cm_centered * Globality_centered +
...
corAR_test_km<-anova(model_SessionKM_3, model_SessionKM_3_no_corr_matrix) # The model with correlation matrix fits significantly better
...
hlmtable<-huxreg("Nullmodell" = null_model_km, 
                 ...
                 "Model 4" = model_SessionKM_3_no_corr_matrix,
                 statistics = NULL, 
                 number_format = 2, 
                 bold_signif = 0.05, 
...
# Extract Residuals
SessionKM_residuals <- hlm_resid(model_SessionKM_3_no_corr_matrix, level=1, include.ls = T) 
########Normality Plots #####
...
model_SessionH_3_no_corr_matrix <- lme(
  fixed = SessionH_lead1 ~ 
   Pride_cm_centered* PA_cm_centered + 
   Pride_cm_centered* Dynamics_centered +
   Pride_cm_centered* Locus_centered + 
...
corAR_test_h <- anova( model_SessionH_3, model_SessionH_3_no_corr_matrix)
### Table Output #####
hlmtable_h<-huxreg("Nullmodell" = null_model_h, 
                   "Model 1" = model_SessionH_1, 
                   "Model 2" = model_SessionH_2, 
                   "Model 3" = model_SessionH_3,
                   "Model 3 without correlation matrix" = model_SessionH_3_no_corr_matrix,
...
The baseline kilometer variable were standardized within their respective sport group, that is, running vs. triathlon. `r missings_percentage` of data were missings. Since most of the incomplete date was due to dropouts, we decided against multiple imputation, following the reccomendations of \[Lit\]. Descriptive statistics were calculated for all variables - descriptive statistics - correlation

We used the nlme-package [@nlme] for the hierarchical linear models, because it allows the use of a covariation matrix. However, using a covariation matrix did not improve the model fit, so no correlation matrix was used. The time-varying predictors pride and positive affect were group-mean centered, and the stable predictors attribution style (globality, dynamics, locus) and implicit motives (achievement, affiliation, power) were grand mean centered. This procedure disentangles the between-person-effects from the within-person-effects. This is in line with recommendations of @Wang2015: The authors emphasize that a within-person effect can be present regardless of the between-person-effect, and vice versa. To illustrate this, it is possible that within a person, a lack of pride after a run predicts a greater running distance in the next session. At the same time, people who less proud on average do not necessarily run more.
...
To account for the possible autocorrelation of repeated measurements, we employed an autoregressive process of order 1 (AR(1)) correlation structure using the corAR1 class from the nlme package in R. This structure was used to model the correlation between observations that are sequentially related in time.
...
No model showed meaningful predictors of running distance. A correlation matrix only slightly and non-significantly improved model fit (*p* = `r corAR_test_h$"p-value"[2] %>% format_p()`). The model with the correlation matrix (Model 3) was retained. 
...
The regression coefficients are shown in @tbl-hlmtable_rpe. The third model with all predictors shows the lowest AIC and log likelihood. T However, this did not result in a better model fit ( *p* = `r intercorr_test_rpe$"p-value"[2] %>% round(2)`), so we used the model without the correlation structure (Model 3).
