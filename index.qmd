...
Model 3 included meaningful predictors and the lowest log-likelihood, it also fit significantly better than the same model without a correlation matrix *p* = `r corAR_test_km$"p-value"[2] %>% format_p()`. Thus, Model 3 was selected for further analysis of statistical assumptions and the discussion. Thus, Model 3 was selected for further analysis of statistical assumptions and the discussion. 
...
On the individual level (level 2), the residuals were not normally distributed (*p* = `r level2.resids.shapirotest_h$p.value %>% round(3)`). No deviation from the assumption of homoscedasticity was present (*p* = `r bp_test_h$p.value[1] %>% round(3) `). 
