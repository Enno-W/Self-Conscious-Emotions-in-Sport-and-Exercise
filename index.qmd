...
# facet_df %>% summarise(across(everything(), ~sum(is.na(.)))) --> The warning is only because of NAs
...
ggplot(data = KM_residuals_level2, aes(.ranef.intercept)) +
  ...
  geom_histogram(aes(y =after_stat(density)), bins=20) + stat_function(fun = dnorm,
                                                                args = list(mean = mean(KM_residuals_level2$.ranef.intercept),
                                                                            sd = sd(KM_residuals_level2$.ranef.intercept)), size=2)

# Shapiro-Wilk Test: Are level 2 residuals normally distributed??
level2.resids.shapirotest <- shapiro.test(KM_residuals_level2$.ranef.intercept)

 # Homoscedasticity in level 2? Extract Level-2 random effects
...
On the individual level (level 2), a shapiro-wilk-test showed that residuals were not normally distributed (*p* `r level2.resids.shapirotest$p.value %>% format_p()`). The distribution of residuals in @fig-sessionkm_assumptions_lvl2 show no serious violation of homoscedasticity. 
...
The marginal and conditional R-squared values, representing the portion of variance explained by the fixed effects and the random effects respectively, were *R^2^~marginal~* = `r Rsquared_rpe[1] %>% round(2)` and *R^2^~conditional~* = `r Rsquared_rpe[2] %>% round(2)`.
 ...
 On the individual level (level 2), the residuals were not normally distributed (*p*  `r level2.resids.shapirotest$p.value %>% format_p()`). No serious deviation from the assumption of homoscedasticity was present in @fig-sessionrpe_assumptions_lvl2.
