...
H1: Positive correlation between achievement motive           and pride
H2: Positive correlation between power motive                 and hubris
H3: Positive correlation between affiliation motive           and pride, 
    negative correlation between affiliation                  and hubris
H4: Positive correlation between hope                         and pride,
    Positive correlation between fear                         and hubris
H5: Positive correlation between internal, variable, controllable,   specific attribution and pride
H6: Positive correlation between internal, stable,   uncontrollable, global   attribution and hubris
H7: Positive correlation between PA parameters                and pride
H8: Negative correlation between PA parameters                and hubris
...
First, implicit motives for achievement, affiliation, and power motives were *z*-standardized. The gender-balanced T-scores for the motive components according to >>>>>>> MC Clelland'S scoring procedure?? <<<<<<<<.  were calculated. The entire data was checked for normal distribution. Applying the Shapiro-Wilk test, the weekly training distance  (*W* = `r shapiro.test(df_study1$WeeklyKM_base)$statistic %>% round(3)`; *p* `r vars_not_normal_with_p_values$WeeklyKM_base`), weekly training duration (*W* = `r shapiro.test(df_study1$WeeklyH_base)$statistic %>% round(3)`; *p* `r vars_not_normal_with_p_values$WeeklyH_base`) and perceived exhaustion (*W* = `r shapiro.test(df_study1$WeeklyRPE_base)$statistic %>% round(3)`; *p* `r vars_not_normal_with_p_values$WeeklyRPE_base`) as well as fear of rejection (*W* = `r shapiro.test(df_study1$Rejection_fear)$statistic %>% round(3)`; *p* `r vars_not_normal_with_p_values$Rejection_fear`) were not normally distributed. Second, descriptive statistics were calculated. Third, due to the small sample size and normality violations, Spearman rank correlations were calculated. For the analysis, Version `r R.Version()$major`.`r R.Version()$minor` of the programming language R [@rlanguage2024] was used.
...
Exploratory analyses revealed significant correlations between pride and positive affect (*Ρ* = `r cor.test(df_study1$Pride_base, df_study1$PA_base,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Pride_base, df_study1$PA_base,method = "spearman")$p.value %>% round (3) %>% format_p()`); pride and fear of rejection (*Ρ* = `r cor.test(df_study1$Pride_base, df_study1$Rejection_fear,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Pride_base, df_study1$Rejection_fear,method = "spearman")$p.value %>% round (3) %>% format_p()`); hubris and negative affect (*Ρ* = `r cor.test(df_study1$Hubris_base, df_study1$NA_base,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Hubris_base, df_study1$NA_base,method = "spearman")$p.value %>% round (3) %>% format_p()`);   weekly training duration and training distance (*Ρ* = `r cor.test(df_study1$WeeklyH_base, df_study1$WeeklyKM_base,method = "spearman")$estimate %>% round (2)`, *p*  `r cor.test(df_study1$WeeklyH_base, df_study1$WeeklyKM_base,method = "spearman")$p.value %>% round (3) %>% format_p()`). The implicit affiliation motive (*Ρ* = `r cor.test(df_study1$Affiliation, df_study1$WeeklyH_base,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Affiliation, df_study1$WeeklyH_base,method = "spearman")$p.value %>% round (3) %>% format_p()`) and dynamic attributions (*Ρ* = `r cor.test(df_study1$Dynamics, df_study1$WeeklyH_base,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Dynamics, df_study1$WeeklyH_base,method = "spearman")$p.value %>% round (3) %>% format_p()`) correlated with training duration. Negative affect correlated with fear of failure (*Ρ* = `r cor.test(df_study1$NA_base, df_study1$Failure_fear,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$NA_base, df_study1$Failure_fear,method = "spearman")$p.value %>% round (3) %>% format_p()`), fear of rejection (*Ρ* = `r cor.test(df_study1$NA_base, df_study1$Rejection_fear,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$NA_base, df_study1$Rejection_fear,method = "spearman")$p.value %>% round (3) %>% format_p()`), locus (*Ρ* = `r cor.test(df_study1$NA_base, df_study1$Locus,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$NA_base, df_study1$Locus,method = "spearman")$p.value %>% round (3) %>% format_p()`) and controllability (*Ρ* = `r cor.test(df_study1$NA_base, df_study1$Controlability_self,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$NA_base, df_study1$Controlability_self,method = "spearman")$p.value %>% round (3) %>% format_p()`). The achievement motive correlated with the implicit affiliation (*Ρ* = `r cor.test(df_study1$Achievement, df_study1$Affiliation,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Achievement, df_study1$Affiliation,method = "spearman")$p.value %>% round (3) %>% format_p()`) and power motives (*Ρ* = `r cor.test(df_study1$Achievement, df_study1$Power,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Achievement, df_study1$Power,method = "spearman")$p.value %>% round (3) %>% format_p()`). Furthermore, fear of failure correlated with fear of rejection (*Ρ* = `r cor.test(df_study1$Failure_fear, df_study1$Rejection_fear,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Failure_fear, df_study1$Rejection_fear,method = "spearman")$p.value %>% round (3) %>% format_p()`), and fear of loss of control (*Ρ* = `r cor.test(df_study1$Failure_fear, df_study1$LossControl_fear,method = "spearman")$estimate %>% round (2)`, *p*   `r cor.test(df_study1$Failure_fear, df_study1$LossControl_fear,method = "spearman")$p.value %>% round (3) %>% format_p()`). Finally, uncontrollable and internal attributions showed a correalation (*Ρ* = `r cor.test(df_study1$Controlability_self, df_study1$Locus,method = "spearman")$estimate %>% round (2)`, *p* `r cor.test(df_study1$Controlability_self, df_study1$Locus,method = "spearman")$p.value %>% round (3) %>% format_p()`).
...
ggplot(plot_df, aes(x = pred, y = ranef_intercept)) +
  ...
  geom_smooth(method = "loess") +
  theme_minimal() +
  labs(x = "Vorhergesagte Level-2-Werte (Prädiktion durch Fixeffekte)",
       y = "Residuen (Random Intercepts)",
       title = "Diagnose: Homoskedastizität und Linearisität auf Level 2")

### Statistical test



bp_test_km <- bptest(ranef_intercept ~ pred, data = plot_df) # Not significant
...
### Statistical test
bp_test_rpe <- bptest(ranef_intercept ~ pred, data = plot_df) # Not significant
...
We excluded participants manually if they reported data on less than two training sessions, if they did not participate in a structured training programme, or if they were neither runners nor triathletes. Subsequent analyses were performed using the R language [@rlanguage2024]. 
 `r missings_percentage_study2`% of all values were missings. 
Since the questionnaires were sent out in weekly intervals, we expected autocorrelation between measurement points. We visually assessed the autocorrelation structure between measurement points: The variograms for all models indicated that autoregressive noise might be present: The semivariance changed substantially with distance between observations. Thus, we included an autocorrelation structure of order 1 (AR1), and compared the model quality with and without a correlation matrix. 
...
In these models, we used interaction terms of pride with positive affect, causal attribution and implicit motives to predict running distance. The running distance was z-standardized within sessions speeds below five km/h, between five and 20 km/h and over 20km/h. We added predictors to the null model in three steps: First, the interaction term of pride and positive affect, second, the interaction terms of pride and attribution styles, and third, interaction terms of pride and implicit motives. The result is shown in @tbl-hlmtable_km.

The intraclass correlation coefficient was *ICC* = `r icc_km[1]$ICC_adjusted %>% round (2)`. 
Regarding autocorrelation between the measurements, the autoregressive parameter was φ = `r autocorr_param_KM_CI_est %>% round(2)`, 95% CI [`r autocorr_param_KM_CI_lower`, `r autocorr_param_KM_CI_upper`], indicating a tendency that higher values were followed by lower values of session distance, and vice versa. 
...
On the individual level (level 2), the residuals were not normally distributed (*p* `r level2.resids.shapirotest$p.value %>% format_p()`). No deviation from the assumption of homoscedasticity was present (*p*  `r bp_test_km$p.value[1] %>% format_p() `). 
...
The interaction terms between pride and globality (*b* = `r b_prideXpa_km %>% round(3)`, *p* `r p_prideXpa_km %>% format_p()`), the affiliation motive (*b* = `r b_prideXaffiliation_km %>% round(2)`, *p* = `r b_prideXaffiliation_km %>% format_p()`) and the power motive (*b* = `r b_prideXpower_km %>% round(2)`, *p* `r b_prideXpower_km %>% format_p()`) were negative: When globality, affiliation or power are high, pride reduces the running distance in the next session. 
...
None of the models identified significant predictors of session duration. With an autoregressive coefficient of φ = `r autocorr_param_H_CI_lower %>% round(2)`, 95% CI [`r autocorr_param_H_CI_lower %>% round(2)`, `r autocorr_param_H_CI_upper %>% round(2)`], modelling autocorrelation did not improve the model fit significantly (*p*  `r corAR_test_h$"p-value"[2] %>% format_p()`). The model with the correlation matrix (Model 3) was retained for further examination. 
...
On the individual level (level 2), the Shapiro-Wilk-test suggests non-normality (*p*  `r level2.resids.shapirotest_h$p.value %>% format_p()`). No deviation from the assumption of homoscedasticity was present (*p*  `r bp_test_h$p.value[1] %>% format_p() `). 
...
The variance inflation factors suggest low multicollinearity [for a discussion see @Obrien2007].
